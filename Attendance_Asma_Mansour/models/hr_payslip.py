...
class HrPayslip(models.Model):
    ...
    attendance_ids = fields.One2many('extra.hours', 'payslip_id', string='Attendances',
    ...
    def _filter_worked_day_lines_values(self, worked_day_lines_values):
        ...
        if self.employee_id.soumis_aux_heures_sup:
            original_work_type = self.env.ref('hr_work_entry.work_input_attendance', raise_if_not_found=False)
    ...
    def _pre_aggregate_work_data(self):
        ...
        if self.employee_id.soumis_aux_heures_sup:
            attendance_to_keep = self.attendance_ids.filtered(lambda a: a.employee_id == self.employee_id
                                                                        and a.hours_end.date() <= self.date_to)
            attendance_to_keep |= self.env['extra.hours'].search([
                ('employee_id', '=', self.employee_id.id),
                ('hours_end', '<=', self.date_to),
                ('payslip_id', '=', False),
            ])
    ...
    def _pre_aggregate_attendance_data(self, work_data, default_workentrytype):
        for attn in self.attendance_ids:
            if attn.extra_hours:
                # Avoid in/outs
                attn_iso = attn.hours_start.isocalendar()
                ...
                if attendance_type in self.struct_id.unpaid_work_entry_type_ids:
                    ...
                    continue
                work_data[attn_iso].append((attendance_type, attn.extra_hours, attn))
        return work_data
